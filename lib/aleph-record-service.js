...
const debug = require('debug')('@natlibfi/aleph-record-caretaker:aleph-record-service');
...
function createAlephRecordService(XServer, credentials) {
  ...
  function saveRecord(base, recordId, record) {
    ...
    const recordInOAI_MARCXML = RecordSerializers.OAI_MARCXML.to(record, {omitDeclaration: true});
