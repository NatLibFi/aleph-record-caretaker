...
function create(alephRecordService, alephFindService, options) {
  ...
  async function handleBibChange(change) {
    ...
    fixedRecord.fields = await utils.serial(fixedRecord.fields.map(field => async () => {
      ...
      if (authorityRecordLinkSubfields.length > 0) {
        ...
        try {
          ...
          logger.log('warn', `[${changeId}] ${bibRecordId}`, error.message);
        }
    ...
    if (!_.isEqual(record.fields, fixedRecord.fields) || !_.isEqual(record.leader, fixedRecord.leader)) {
      logger.log('debug', `[${changeId}] The record ${bibRecordId} has changed.`);
      ...
      logger.log('info', `[${changeId}] Previous values in ${bibRecordId}}: ${patch.prev}`);
      logger.log('info', `[${changeId}] Next values in ${bibRecordId}: ${patch.next}`);

      const wasRecentChange = recentChangesManager.checkAndUpdateRecentChanges(change.library, change.recordId, patch);
      if (wasRecentChange) {
        logger.log('info', `[${changeId}] I recently made this change to ${bibRecordId}, so skipping this for now.`);
        return;
      }
      ...
      if (options.noOperation || options.noOperationBibChange) {
        ...
        logger.log('info', `[${changeId}] Saving changed bib ${bibRecordId} record.`);
        try {
      ...
      logger.log('debug', `[${changeId}] The record ${bibRecordId} was not changed.`);
    }
  }
