      ...
      if (authorityRecordLinkSubfields.length > 0) {
        ...
        if (supportedLinks.length === 1) {
          ...
          if (authorizedFields.length !== 1) {
            const error = new Error('Could not parse authorized portion from authority record');
            error.authorityRecordId = recordId;
            error.recordId = change.recordId;
            throw error;
          }
