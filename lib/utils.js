...
const debug = require('debug')('@natlibfi/aleph-record-caretaker:utils');

// Melinda/Aleph record ids are 9 digits wide (zero-padded).
const MELINDA_ID_WIDTH = 9;

function padMelindaId(recordId) {
  return _.padStart(String(recordId), MELINDA_ID_WIDTH, '0');
}
...
function readEnvironmentVariable(name, defaultValue, opts) {
  ...
  if (process.env[name] === undefined) {
    ...
    console.log(`No environment variable set for ${name}, using default value: ${loggedDefaultValue}`); //eslint-disable-line no-console
  }
...
function RecentChangesManager(recentChangeCooldownMs = 20000) {
  ...
  function purgeOldChanges(now) {
    ...
    Object.keys(recentChanges).forEach(changeHash => {
      ...
      if (now - changeDate > recentChangeCooldownMs) {
        ...
        delete(recentChanges[changeHash]);
      }
    });
  }
  ...
  return {
    ...
    purgeOldChanges,
    getEntryCount: () => Object.keys(recentChanges).length
  };
}

function accumulate(count, fn, fnContext) {
  let callCount = 0;
