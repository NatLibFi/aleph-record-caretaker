...
function findAuthorizedPortion(recordType, recordField) {
  if (recordType === RecordType.AUTH) {
    ...
    switch(recordField.tag) {
      case '100': isAuthorizedSubfield = (sub) => !_.includes(['e', '6', '8', '9', '0'], sub.code) && !isSpecifierSubfield(sub) && !isTitlePortion(sub); break;
      case '110': isAuthorizedSubfield = (sub) => !_.includes(['e', '6', '8', '9', '0'], sub.code) && !isSpecifierSubfield(sub) && !isTitlePortion(sub); break;
      case '111': isAuthorizedSubfield = (sub) => !_.includes(['j', '6', '8', '9', '0'], sub.code) && !isSpecifierSubfield(sub) && !isTitlePortion(sub); break;
...
function updateAuthorizedPortion(recordType, recordField, authorizedPortion) {
  if (recordType === RecordType.AUTH) {
    ...
    updatedRecordField.subfields.splice.bind(updatedRecordField.subfields, currentAuthorizedPortion.range.start, currentAuthorizedPortion.range.length).apply(null, _.cloneDeep(authorizedPortion.subfields));
