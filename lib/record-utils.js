...
const {AuthorizedPortion} = require('@natlibfi/melinda-marc-record-utils');
const RecordType = {
  AUTH: 'AUTH',
  BIB: 'BIB'
};
...
function extractAuthorizedPortion(authorityRecord) {
  ...
  return AuthorizedPortion.findAuthorizedPortion(AuthorizedPortion.RecordType.AUTH, authorizedFields[0]);
}
// NOTE: @natlibfi/marc-record has no isDeleted method (marc-record-js did), so the
// deletion check is replicated here. This matches the original marc-record-js
// isDeleted() semantics (STA/DEL as datafields, or leader status 'd').
function isRecordDeleted(record) {
  if (record.containsFieldWithValue('STA', [{code: 'a', value: 'DELETED'}])) {
    return true;
  }
  if (record.containsFieldWithValue('DEL', [{code: 'a', value: 'Y'}])) {
    return true;
  }
  if (record.leader && record.leader.length >= 6) {
