...
function selectPersonFields(record) {
  const fields = record.fields.filter(field => _.includes(['100', '600', '700'], field.tag));
  const personFields = fields
    .filter(keepFennicaOrYsa600)
    .map(field => {

      const nameFields = field.subfields.filter(subfield => _.includes(['a', 'b', 'c', 'd', 'g', 'j', 'q'], subfield.code));
...
function selectCorporateNameFields(record) {
  const fields = record.fields.filter(field => _.includes(['110', '610', '710'], field.tag));
  return fields
    .filter(keepFennicaOrYsa600)
    .map(field => {

      const nameFields = field.subfields.filter(subfield => _.includes(['a', 'b', 'c', 'd', 'g', 'n'], subfield.code));
      return { field, nameFields };
    });
}

function selectMeetingNameFields(record) {
  const fields = record.fields.filter(field => _.includes(['111', '611', '711'], field.tag));
  return fields
    .filter(keepFennicaOrYsa600)
    .map(field => {
...
function selectFieldForLinkingWithZero(record, queryTerms) {
  ...
  const names = selectPersonFields(record);
  const orgs = selectCorporateNameFields(record);
  const convs = selectMeetingNameFields(record); 

  const matches = _.concat(names, orgs, convs).filter(name => {
    const normalized = name.nameFields.map(sub => ({ code: sub.code, value: normalizeForHeadingQuery(sub.value) }));
...
function findAuthorizedPortion(recordType, recordField) {
  if (recordType === RecordType.AUTH) {
    ...
    switch(recordField.tag) {
      case '100': isAuthorizedSubfield = (sub) => !_.includes(['6', '8', '0'], sub.code); break;
      case '110': isAuthorizedSubfield = (sub) => !_.includes(['6', '8', '0'], sub.code); break;
      case '111': isAuthorizedSubfield = (sub) => !_.includes(['6', '8', '0'], sub.code); break;
      
      case '500': isAuthorizedSubfield = (sub) => !_.includes(['i', 'w', '4', '5', '6', '8', '9', '0'], sub.code); break;
      case '510': isAuthorizedSubfield = (sub) => !_.includes(['i', 'w', '4', '5', '6', '8', '9', '0'], sub.code); break;
      case '511': isAuthorizedSubfield = (sub) => !_.includes(['i', 'w', '4', '5', '6', '8', '9', '0'], sub.code); break;
      
      case '700': isAuthorizedSubfield = (sub) => !_.includes(['i', 'w', '4', '0'], sub.code); break;
      case '710': isAuthorizedSubfield = (sub) => !_.includes(['i', 'w', '4', '0'], sub.code); break;
      case '711': isAuthorizedSubfield = (sub) => !_.includes(['i', 'w', '4', '0'], sub.code); break;
    ...
    return {
      ...
      tag: recordField.tag,
      subfields: _.clone(recordField.subfields).filter(isAuthorizedSubfield),
      range: {
  ...
  if (recordType === RecordType.BIB) {
    //TODO
    throw new Error('findAuthorityPoriton not implemented for BIB records');
  }
...
function updateAuthorizedPortion(recordType, recordField, authorizedPortion) {
  if (recordType === RecordType.AUTH) {
    ...
    const updatedRecordField = _.clone(recordField);
  ...
  if (recordType === RecordType.BIB) {
    throw new Error('updateAuthorizedPortion not implemented for BIB records');
  }
  throw new Error(`Invalid record type ${recordType}`);
}
