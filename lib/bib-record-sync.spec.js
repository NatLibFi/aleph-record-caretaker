...
describe('parseAuthorityLinkGlobal', function() {
   ...
   it('should throw on values containing multiple (base)id pairs', () => {
       ...
       expect(function () {
         parseAuthorityLinkGlobal("(FIN11)000123456,(FIN11)000123457", options);
       }).to.throw(Error, /multiple \(base\)id pairs/);

       expect(function () {
         parseAuthorityLinkGlobal("(FIN11)000123456,(FIN10)000123456", options);
       }).to.throw(Error, /multiple \(base\)id pairs/);
...
describe('BibRecordSyncService', () => {
  ...
  it('should reject with an error if single field contains multiple links with same base', () => {
    ...
    return pro.then(expectFailure, (error) => {
      expect(error.message).to.equal('TST01 / 00001 Record contains multiple links to supported bases (TST10, TST10). Unable to determine which one to use for updating the authorized portion.');
    });
