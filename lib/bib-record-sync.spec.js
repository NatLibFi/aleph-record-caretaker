...
describe('BibRecordSyncService', () => {
  ...
  it('should reject with an error if single field contains multiple links with same base', () => {
    ...
    return pro.then(expectFailure, (error) => {
      expect(error.message).to.equal('Record 00001 contains multiple links to supported bases (TST10, TST10). Unable to determine which one to use for updating the authorized portion.');
    });
