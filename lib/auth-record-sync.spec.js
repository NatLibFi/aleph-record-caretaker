...
describe('AuthRecordSyncService', () => {
  ...
  describe('for linked bib records', () => {
    ...
    it('should not stop when one of the records fail', async () => {
      ...
      const error = _.get(errorMessages, '[0][3]');
  ...
  describe('for linked auth records', () => {
    ...
    it('should not save a linked auth record for a change recently made by the caretaker', async () => {
      ...
      // Saving is stubbed, so the loaded record still contains the old field and
      // the same change is computed again. It must be skipped as a recent change.
      await recordSyncService.handleAuthChange(fakeChange);
      expect(recordServiceStub.saveRecord.callCount).to.equal(1, 'SaveRecord was called again for a recent change');

      const logMessages = _.range(0, logger.log.callCount).map(i => logger.log.getCall(i)).map(i => _.get(i, 'args'));
      const recentMessages = logMessages.filter(msg => _.get(msg, '[1]') && _.get(msg, '[1]').includes('I recently made this change'));
...
describe('AuthRecordSyncService with plain base links', () => {

  // Production-shaped config: baseMap keys are namespace identifiers (ISILs), values are Aleph bases.
  // Plain base links use the Aleph base (TST10) instead of the namespace key (TST10-N).
  const baseMap = {
    'TST01-S': 'TST01',
    'TST10-N': 'TST10'
  };
  ...
  it('should update the bib record fields linked with a plain Aleph base', async () => {
  ...
  it('should update the bib record fields linked with a plain Aleph base even if link subfield has mistaken punctuation', async () => {
  ...
  it('should not update the bib record fields linked with the plain base of another library', async () => {

    fakeBibRecord.appendField(RecordUtils.stringToField('100    ‡aAakkula, I.‡tcontent‡0(TST01)90001'));
    fakeAuthRecord.appendField(RecordUtils.stringToField('100    ‡aAakkula, Immo.‡tcontent'));
    findServiceStub.findRecordsLinkedToAgentRecord.withArgs('TST01', '90001').resolves(['00001']);
  ...
  it('should copy the name from the authority record and update the linked auth record fields linked with a plain base', async () => {

    fakeAuthRecord.appendField(RecordUtils.stringToField('100    ‡aAakkula, Immo,‡d1934-1992‡0(TST10-N)90001'));
    const fakeLinkedAuthRecord = new MarcRecord(fakeAuthRecord);
    fakeLinkedAuthRecord.fields = fakeLinkedAuthRecord.fields.filter(field => field.tag !== '100');
    fakeLinkedAuthRecord.appendField(RecordUtils.stringToField('500    ‡aAakkula, I.,‡0(TST10)90001'));
...
describe('AuthRecordSyncService error and no-op handling', () => {
  ...
  function createService(options) {
    return AuthRecordSyncService.create(recordServiceStub, findServiceStub, {
      bibRecordBase: 'TST01',
      agentRecordBase: 'TST10',
      baseMap,
      urnBaseMap,
  ...
  beforeEach(() => {
    ...
    // handleAuthChange also queries the agent base for linked auth records
    findServiceStub.findRecordsLinkedToAgentRecord.resolves([]);
  ...
  it('should throw when created without punctuationRulesForAuthRecord', () => {
    expect(() => AuthRecordSyncService.create(recordServiceStub, findServiceStub, {
      bibRecordBase: 'TST01',
      agentRecordBase: 'TST10',
      baseMap,
      urnBaseMap,
      urnResolverPrefix,
      logger: { log: sinon.spy() }
    })).to.throw(Error, 'Missing punctuation rules for authority records');
  });

  it('should report a no-op when the record changed but noOperation is set', async () => {
    fakeBibRecord.appendField(RecordUtils.stringToField('100    ‡aAakkula, I.‡tcontent‡0(TST10)90001'));
    fakeAuthRecord.appendField(RecordUtils.stringToField('100    ‡aAakkula, Immo.‡tcontent'));
    findServiceStub.findRecordsLinkedToAgentRecord.withArgs('TST01', '90001').resolves(['00001']);
    recordServiceStub.loadRecord.withArgs('TST01', '00001').resolves(fakeBibRecord);
    ...
    const recordSyncService = createService({ noOperation: true });

    await recordSyncService.handleAuthChange({ recordId: '90001', library: 'TST10' });

    // the record changed, but the no-operation flag prevented a save
    expect(recordServiceStub.saveRecord.callCount).to.equal(0);
  });

  it('should report an error when saving the record fails', async () => {
    recordServiceStub.saveRecord.rejects(new Error('save failed'));
    ...
    const recordSyncService = createService({});
