...
describe('AuthRecordSyncService', () => {
  ...
  describe('for linked bib records', () => {
    ...
    it('should not stop when one of the records fail', async () => {
      ...
      expect(logMessages).to.deep.include(['info', 'Loading bib record TST01 / 00001']);
      expect(logMessages).to.deep.include(['info', 'Loading bib record TST01 / 00002']);
      expect(logMessages).to.deep.include(['info', 'Loading bib record TST01 / 00003']);
      
      const errorMessages = logMessages.filter(msg => msg[0] === 'error');
      const error = _.get(errorMessages, '[0][1]');
      expect(error.name).to.equal('PunctuationError');
      expect(error.message).to.equal('Unknown subfield code H');
      
      expect(logMessages).to.deep.include(['info', 'Record is deleted, skipping.']);
