...
async function processBibRecord(context, bibId, authorizedPortion, change, changeId, stats) {
  ...
  try {
    ...
    if (RecordUtils.isRecordDeleted(bibRecord)) {
    ...
    debug(`[${changeId}] Updating fields in ${bibRecordBase} / ${bibId} with ‡0 link to ${change.library} / ${change.recordId} (ISIL, plain base or URN form)`);
    ...
    if (syncCommon.recordChanged(bibRecord, fixedRecord)) {
      logger.log('info', `[${changeId}] ${bibRecordBase} / ${bibId} The record has changed.`);
      stats.changes = stats.changes + 1;

      const patch = syncCommon.createPatch(bibRecord.fields, fixedRecord.fields);
      syncCommon.logPatch(context.logger, changeId, bibRecordBase, bibId, patch);
...
async function processLinkedAuthRecord(context, linkedAuthId, authorizedPortion, change, changeId, stats) {
  ...
  try {
    ...
    if (RecordUtils.isRecordDeleted(linkedAuthRecord)) {
    ...
    if (syncCommon.recordChanged(linkedAuthRecord, fixedRecord)) {
      logger.log('info', `[${changeId}] The record has changed.`);
      stats.changes = stats.changes + 1;

      const patch = syncCommon.createPatch(linkedAuthRecord.fields, fixedRecord.fields);
      syncCommon.logPatch(context.logger, changeId, agentRecordBase, linkedAuthId, patch);
