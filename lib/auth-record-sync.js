...
async function handleAuthChange(context, change) {
  ...
  const bibIdList = await context.alephFindService.findRecordsLinkedToAgentRecords(bibRecordBase, change.recordId);
  ...
  const linkedAgentAuthIdList = await context.alephFindService.findRecordsLinkedToAgentRecords(agentRecordBase, change.recordId);
