...
function create(alephRecordService, alephFindService, options) {
  ...
  async function handleAuthChange(change) {
    ...
    logger.log('info', `[${changeId}] Handling changed auth record ${change.recordId}`, JSON.stringify(change));
    ...
    if (authorityRecord.isDeleted()) {
      logger.log('info', `[${changeId}] Record ${recordId} is deleted, skipping.`);
      return;
    }

    // TODO: this supports only agent authorities. Expand to support also subject authorities
    debug(`[${changeId}] extracting authorized portion from auth record ${recordId}`);
    const authorizedFields = authorityRecord.fields.filter(field => _.includes(['100', '110', '111'], field.tag));

    if (authorizedFields.length !== 1) {
      throw new Error(`Could not parse authorized portion from authority record ${recordId}`);
    }
    ...
    debug(`[${changeId}] Authorized portion in ${recordId} is ${JSON.stringify(authorizedPortion)}`);
    ...
    logger.log('info', `[${changeId}] Found ${bibIdList.length} bib records to handle.`);
	...
	const urnQuery2=createUrnQueryPadded(change,options);  
        ...
        if (!_.isEqual(bibRecord.fields, fixedRecord.fields) || !_.isEqual(bibRecord.leader, fixedRecord.leader)) {
          logger.log('info', `[${changeId}] The record ${bibId} has changed.`);
          ...
          logger.log('info', `[${changeId}] Previous values in ${bibId}}: ${patch.prev}`);
          logger.log('info', `[${changeId}] Next values in ${bibId}: ${patch.next}`);
          ...
          if (options.noOperation) {
            logger.log('info', `[${changeId}] no-operation flag is set, not saving record ${bibId}`);
          } else {
            logger.log('info', `[${changeId}] Saving changed bib record ${bibId}.`);
            await alephRecordService.saveRecord(bibRecordBase, bibId, fixedRecord);
            logger.log('info', `[${changeId}] Record ${bibId} was saved succesfully.`);
          }
          
        } else {
          logger.log('debug', `[${changeId}] The record ${$bibId} was not changed.`);
        }
      } catch(error) {
    ...
    logger.log('info', `[${changeId}] Found ${linkedAgentAuthIdList.length} linkead auth records to handle.`);
    ...
    for (const linkedAuthId of linkedAgentAuthIdList) {
      try {
        ...
        debug(`[${changeId}] checking and maybe resetting authrozied portion from linked auth ${agentRecordBase} / ${linkedAuthId} record fields`);
	...
	const urnQuery = createUrnQuery(change,options);
	  ...
	  if (field.subfields === undefined) return field;
        ...
        if (!_.isEqual(linkedAuthRecord.fields, fixedRecord.fields) || !_.isEqual(linkedAuthRecord.leader, fixedRecord.leader)) {
          logger.log('info', `[${changeId}] The record ${agentRecordBase} / ${linkedAuthId} has changed.`);
	  ...
	  const {a, b} = utils.deepDiff(linkedAuthRecord.fields, fixedRecord.fields);
          ...
          logger.log('info', `[${changeId}] Previous values in ${agentRecordBase} /  ${linkedAuthId}}: ${patch.prev}`);
          logger.log('info', `[${changeId}] Next values in ${agentRecordBase} / ${linkedAuthId}: ${patch.next}`);
		
          if (options.noOperation) {
            logger.log('info', `[${changeId}] no-operation flag is set, not saving record ${agentRecordBase} / ${linkedAuthId}`);
          } else {
          ...
          logger.log('debug', `[${changeId}] The record ${agentRecordBase} / ${linkedAuthId}was  not changed.`);
        }
      } catch(error) {
