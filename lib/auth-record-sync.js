...
function create(alephRecordService, alephFindService, options) {
  ...
  async function handleAuthChange(change) {
    ...
    const bibCount = bibIdList.length;
    let bibNoChanges = 0;
    let bibChanges = 0;
    let bibSaves = 0;
    let bibErrors = 0;
    ...
    for (const bibId of bibIdList) {
      try {
        ...
        fixedRecord.fields = bibRecord.fields.map(field => {
          ...
          //debugDev(`[${changeId}] ${bibRecordBase} / ${bibId} Field after fixPunctuationFromBibField ${RecordUtils.fieldToString(updatedField)}`);
          const fixedField = fieldFixPunctuation(updatedField);
        ...
        if (!_.isEqual(bibRecord.fields, fixedRecord.fields) || !_.isEqual(bibRecord.leader, fixedRecord.leader)) {
          ...
          bibNoChanges = bibNoChanges + 1;
        }
      } catch(error) {
        bibErrors = bibErrors + 1;
        logger.log('error', `[${changeId}] ${bibRecordBase} / ${bibId}`, error);
      }
    }
    logger.log('info', `[${changeId}] Handled bibs linked to ${change.recordId}: BibCount: ${bibCount}. bibNoChanges: ${bibNoChanges}, bibChanges: ${bibChanges}, bibSaves: ${bibSaves}, bibErrors ${bibErrors}, bibNoOps: ${bibNoOps}`);
    ...
    const linkedAgentCount = linkedAgentAuthIdList.length;
    debug(`[${changeId}] Linked to ${change.recordId} auth records [${linkedAgentAuthIdList.join(', ')}]`);
    
    let linkedAgentNoChanges = 0;
    let linkedAgentChanges = 0;
    let linkedAgentSaves = 0;
    let linkedAgentErrors = 0;
    let linkedAgentNoOps = 0;
    ...
    for (const linkedAuthId of linkedAgentAuthIdList) {
      try {
        ...
        if (linkedAuthRecord.isDeleted()) {
          ...
          linkedAgentNoChanges = linkedAgentNoChanges + 1;
          continue;
        }
        ...
        if (!_.isEqual(linkedAuthRecord.fields, fixedRecord.fields) || !_.isEqual(linkedAuthRecord.leader, fixedRecord.leader)) {
          ...
          linkedAgentChanges = linkedAgentChanges + 1;
          ...
          if (options.noOperation) {
            ...
            linkedAgentNoOps = linkedAgentNoOps + 1;
          } else {
            ...
            linkedAgentSaves = linkedAgentSaves + 1;
          }
        ...
        linkedAgentErrors = linkedAgentErrors + 1;
      }
    }
    logger.log('info', `[${changeId}] Handled linkedAgents linked to ${change.recordId}: linkedAgentCount: ${linkedAgentCount}. linkedAgentNoChanges: ${linkedAgentNoChanges}, linkedAgentChanges: ${linkedAgentChanges}, linkedAgentSaves: ${linkedAgentSaves}, linkedAgentErrors ${linkedAgentErrors}, linkedAgentNoOps: ${linkedAgentNoOps}`);
  }
