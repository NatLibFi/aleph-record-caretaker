...
describe('utils', () => {
  ...
  describe('padMelindaId', () => {
    it('should pad short record ids to MELINDA_ID_WIDTH with leading zeros', () => {
      expect(utils.padMelindaId('90001')).to.equal(`000090001`);
      expect(utils.padMelindaId('1')).to.equal(`000000001`);
    });

    it('should leave ids that are already MELINDA_ID_WIDTH wide unchanged', () => {
      const id = '123456789';
      expect(utils.padMelindaId(id)).to.equal(id);
      expect(utils.padMelindaId('123456789')).to.have.lengthOf(utils.MELINDA_ID_WIDTH);
    });

    it('should not truncate ids longer than MELINDA_ID_WIDTH', () => {
      expect(utils.padMelindaId('1234567890')).to.equal('1234567890');
    });

    it('should coerce non-string ids to strings before padding', () => {
      expect(utils.padMelindaId(90001)).to.equal('000090001');
    });

    it('should expose MELINDA_ID_WIDTH as 9', () => {
      expect(utils.MELINDA_ID_WIDTH).to.equal(9);
    });
  });
