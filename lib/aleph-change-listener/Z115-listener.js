...
const debug = require('debug')('@natlibfi/aleph-record-caretaker:aleph-change-listener:Z115-Listener');
...
async function getChangesSinceId(base, connection, sinceChangeId) {
  ...
  debug(`Got rows ${rows.length}`)
  const changes = rows.map(parseZ115Row);
  debug(`Got changes ${changes.length}`)
...
function parseZ115Row(row) {
  ...
  return {
    ...
    date: utils.parseDate(Z115_TODAY_DATE, Z115_TODAY_TIME),
