...
function create(base, stashPrefix = 'stash') {
  const debug = createDebug(`@natlibfi/aleph-record-caretaker:aleph-change-listener:Z106-Listener-for${base}`);
  ...
  async function getChangesAtDate(connection, sinceDate) {
    ...
    debug(`getChangesAtDate: We got results.`);
    //debug(`getChangesAtDate: We got results. ${JSON.stringify(result.resultSet)}`);

    const rows = await readAllRowsLocal(result.resultSet);
    debug(`getChangesAtDate: We got rows.`);
  ...
  async function readAllRowsLocal(resultSet, counter = 0, rows = []) {
    counter = counter + 1;
    debug(`readAllRowsLocal: row ${counter}`);
    //debug(`Result set: ${JSON.stringify(resultSet)}`);
    const nextRow = await resultSet.getRow();
    debug(JSON.stringify(nextRow));
    if (nextRow === null || nextRow === undefined || !nextRow) {
      debug(`Did not find a row.`);
      await resultSet.close();
      return rows;
    }
    debug(`Found a row.`);
    rows.push(nextRow);
    return readAllRowsLocal(resultSet, counter, rows);
  }
...
function parseZ106Row(row, base = '???') {
  const debug = createDebug(`Z106-Listener-for${base}:parseZ106Row`);
  debug(`Row: ${JSON.stringify(row)}`);
  if (row === undefined || row === null || !row) {
    debug(`WARN Row does not exist!`);
    return {};
  }
