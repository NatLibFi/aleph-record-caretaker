...
const debug = require('debug')('@natlibfi/aleph-record-caretaker:aleph-find-service');
const utils = require('./utils');
const MAX_ENTRIES = 99;
function create(X_SERVER) {
  async function findBibRecordsLinkedToAgentRecord(authorityRecordId) {
    return findRecordsLinkedToAgentRecord('fin01', authorityRecordId);
  }
  async function findBibRecordsLinkedToSubjectRecord(authorityRecordId) {
    return findRecordsLinkedToSubjectRecord('fin01', authorityRecordId);
  }
  async function findRecordsLinkedToAgentRecord(base, authorityRecordId) {
    const normalizedRecordId = utils.padMelindaId(authorityRecordId);
    const result = await queryIndex(base, 'ANAID', normalizedRecordId);
    return result.recordIds;
  }
  async function findRecordsLinkedToSubjectRecord(base, authorityRecordId) {
    const normalizedRecordId = utils.padMelindaId(authorityRecordId);
  ...
  async function fetchItems(resultSet) {
    async function fetchItemsIterate(resultSet, recordIds, offset) {
      ...
      const recordIdsPage = records.map(record => _.get(record, 'doc_number[0]'));
      debug(`This fetch (${sessionId}): ` + recordIdsPage.toString());
      recordIds = recordIds.concat(recordIdsPage);
      const foundIdsCount = recordIds.length;
      debug(`Currently found: ${foundIdsCount} records`);
      if (end <= resultSet.noEntries) {
        return fetchItemsIterate(resultSet, recordIds, end + 1 );
      }
        // NOTE: This returns all recordIds but only last sessionId
        debug(`Returning: ` + recordIds.toString());
        return { recordIds, sessionId  };
    }
    const recordIds=[];
    return fetchItemsIterate(resultSet,recordIds,1);
  }
  return {
    findBibRecordsLinkedToAgentRecord,
    findBibRecordsLinkedToSubjectRecord,
    findRecordsLinkedToAgentRecord,
    findRecordsLinkedToSubjectRecord,
    queryIndex
  };
}
module.exports = {
