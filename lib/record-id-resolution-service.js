...
const debug = require('debug')('@natlibfi/aleph-record-caretaker:record-id-resolution-service');
...
function create(XServer, alephUrl, base) {
  ...
  */
  // NOTE: copied from marc-record-js, since @natlibfi/marc-record has no isDeleted method
  function isRecordDeleted(record) {
    if (record.containsFieldWithValue('STA', 'a', 'DELETED')) {
      return true;
    }
    if (record.containsFieldWithValue('DEL', 'a', 'Y')) {
      return true;
    }
